--Replace Volume With Augmented Table
CREATE TABLE tm_new_data.rf_unseen_adjacent_240611 AS 
SELECT DISTINCT  
...
LEFT JOIN 
(SELECT *,'ML' AS SOURCE FROM tm_new_data.rf_unseen_adjacent_240611) aug_cur
ON aug_cur.isg_osm_id = nhv.isg_osm_id 
LEFT JOIN 
(SELECT *, 'ML' AS SOURCE FROM tm_new_data.rf_unseen_adjacent_240611) aug_next
ON aug_next.isg_osm_id = nhv.next_isg_osm_id 
LEFT JOIN 
(SELECT *,'ML' AS SOURCE FROM  tm_new_data.rf_unseen_adjacent_240611) aug_prev
ON  aug_prev.isg_osm_id = nhv.prev_isg_osm_id;
CREATE INDEX  ON tm_new_data.rf_unseen_adjacent_240611 USING btree (isg_osm_id);
CREATE INDEX  ON tm_new_data.rf_unseen_adjacent_240611 USING btree (next_isg_osm_id);
CREATE INDEX  ON tm_new_data.rf_unseen_adjacent_240611 USING btree (prev_isg_osm_id);
CREATE INDEX  ON tm_new_data.rf_unseen_adjacent_240611 USING btree (next_connected_count);
CREATE INDEX  ON tm_new_data.rf_unseen_adjacent_240611 USING btree (prev_connected_count);

--gis plotting
drop table if exists gis_tables.hour_8_volume_FC_1_2_3_rf_next;
create table gis_tables.hour_8_volume_FC_1_2_3_rf_next
as
select distinct isg_osm_id,cur_fc,highway_id,countyid,region,roadwaytype_id,
next_connected_count,prev_connected_count,cur_lanes,cur_maxspeed,cur_final_place,volume,"source",geom
from tm_new_data.rf_unseen_adjacent_240611 where volume > 0;
create index on gis_tables.hour_8_volume_FC_1_2_3_rf_next(isg_osm_id);

--+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++


--adjacent geh multi previous
create table tm_new_data.nys_hour_8_volume_240530_rf_prev_unseen_adjancent_geh_check as
...
from tm_new_data.nys_hour_8_volume_240530_rf_prev cur,
tm_new_data.nys_hour_8_volume_240530_rf_prev conn
...
--adjacent geh multi next
insert into tm_new_data.nys_hour_8_volume_240530_rf_next_unseen_adjancent_geh_check
...
from tm_new_data.nys_hour_8_volume_240530_rf_next cur,
tm_new_data.nys_hour_8_volume_240530_rf_next conn
...
from
	...
	from tm_new_data.nys_hour_8_volume_240530_rf_next where volume>0 and next_volume>0 and next_connected_count =1 and next_pcc = 1) geh,
(select isg_osm_id,source from tm_new_data.nys_hour_8_volume_240530_rf_next  where prev_connected_count =1 ) nxt
